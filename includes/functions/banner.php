...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    ...
    if (tep_not_null($banner['banners_html_text'])) {
      ...
      $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '">' . tep_image(DIR_WS_IMAGES . 'banners/' . $banner['banners_image'], $banner['banners_title'], '100%', '', 'class="img-responsive"') . '</a>';
    }
