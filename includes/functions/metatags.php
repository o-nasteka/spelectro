...
function metatags() {
	...
	if (is_array($cPath_array)) {
		...
		if (isset($_GET['products_id'])) {
			...
			$salida.=($ProductoNombre.' a ' .$precio.''.$EnvioGratis.' &gt; ');
			
			$keywords.=ucfirst(strtolower($ProductoNombre.', '));
		...
		$keywords.=strtolower(get_category_name($cPath_array_reverse[0])).' de '.strtolower(get_category_name($cPath_array_reverse[1])).', ';
		...
		if ($producto==false) {
			$ProductoDescripcion=strtolower(get_category_name($cPath_array_reverse[0])).strtolower(get_category_name($cPath_array_reverse[0])).' de '.strtolower(get_category_name($cPath_array_reverse[1])). '('.strtolower(get_category_name($cPath_array_reverse[2]).')');
		}
		...
		if (isset($_GET['manufacturers_id'])) {
			...
			$keywords.=ucfirst(strtolower($FabricanteNombre.', ')).' marca';
			$ProductoDescripcion = 'Productos de la marca '.$FabricanteNombre.',productos '.$ProductoNombre.' baratos';
		}
		...
		$salida.='<meta name="keywords" content="'.$ProductoNombre.'" />
		'."\n";
		$salida.='<meta name="description" content="'.$ProductoNombre.'" />
		'."\n";
	}
	...
	else
	{
		...
		echo '<meta name="description" content="Repuestos , Pantallas y Flex para Móviles y Tablet Samsung iPhone iPad Sony Xperia LG Nokia BQ Motorola HTC Huawei Xiaomi. Recambios de baterías, tactil, LCD" />' ."\n";
