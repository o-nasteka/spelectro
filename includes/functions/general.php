...
// Parse the data used in the html tags to ensure the tags will not break
  ...
  function tep_sanitize_string($string) {
    $string = preg_replace('/ +/i', ' ', trim($string));
...
// If insufficent stock is available return an out of stock message
  function tep_check_stock($products_id, $products_quantity) {
    ...
    if ($stock_left < 0) {
      $out_of_stock = '<span class="label label-default">' . STOCK_MARK_PRODUCT_OUT_OF_STOCK . '</span>';
    }
...
// TABLES: address_format
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    $nif = tep_output_string_protected($address['nif']);
...
// Text Mode
    ...
    if ( (ACCOUNT_COMPANY == 'true') && (tep_not_null($company)) ) {
      $address = $company . $cr . $address. $cr . "N.I.F. ". $nif;
    }
...
// Break up $search_str on whitespace; quoted string will be reconstructed later
    $pieces = preg_split('/[[:space:]]+/i', $search_str);
    $objects = array();
    $tmpstring = '';
    $flag = '';
...
if($newstring){
return $domain . preg_replace( '/' . $search . '/i', $replace, $string) . $newstring;
}else{
return $domain . preg_replace( '/' . $search . '/i', $replace, $string);
}
}
// eof - canonical tag
...
include(DIR_WS_FUNCTIONS . 'metatags.php');
?>
