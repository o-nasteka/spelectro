...
<?php
  if ($first != (int)$_GET['products_id']) {
		echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, "$fPath&products_id=$previous") . '">' . tep_image_button('button_prev.png', ALT_PREVIOUS_PRODUCT) . '</a>';
  } else {
	 echo tep_image_button('button_prev.png', ALT_PREVIOUS_PRODUCT); 
  }
	 echo PREV_NEXT_PRODUCT . $position . PREV_NEXT_OF . $counter; 
  if ($last != (int)$_GET['products_id']) {
		 echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, "$fPath&products_id=$next_item") . '">' . tep_image_button('button_next.png', ALT_NEXT_PRODUCT) . '</a>'; 
  } else {
	echo tep_image_button('button_next.png', ALT_NEXT_PRODUCT);
  }
  if ($cat_show == 'true') {
		echo $prev_next_in; 
  }
