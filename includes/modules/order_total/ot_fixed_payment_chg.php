  ...
  class ot_fixed_payment_chg {
    ...
    function process() {
      ...
      if ($od_amount != 0) {
        ...
        $this->output[] = array('title' => ($payment == 'cod' ? MODULE_FIXED_PAYMENT_CHG_TITLE_COD : MODULE_FIXED_PAYMENT_CHG_TITLE_PPL ) . ':',
...
//Arreglo para que contrarrembolso MRW cobre minimo 3 Euros
          if ($payment == 'cod') {  //Sustituir 'cod2' por el codigo de la forma de pago que corresponda a MRW'
			if ($od_am_percentage + $od_am_fixed < 3) { //Sustituir el 3 por la cantidad minima sin IVA
				$od_am_percentage = 3; //Sustituir el 3 por la cantidad minima sin IVA
				$od_am_fixed = 0;
			}
          }
