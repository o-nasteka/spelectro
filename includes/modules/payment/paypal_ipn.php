...
// class methods
    ...
    function changePostTest()
    {
	// Obtenemos los datos
	$aDatos = $this->process_button(true);

	// Comprobamos dato a dato que nos envian del post y los comparamos
	foreach( $aDatos as $key => $value)
	{
		if( $_POST[$key] && $value != $_POST[$key] )
			tep_redirect( 'logoff.php' );
	}
    }
    
    function process_button($bReturn = false) {
...
//------insert customer choosen option eof ----
	  ...
	  //ACTUALIZAMOS EL METODO DE ENVIO EN EL PEDIDO
	  for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
            ...
            tep_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array,'update','orders_id='.(int)$order_id.' and class="'.$order_totals[$i]['code'].'"');
          }
...
if (EMAIL_USE_HTML == 'true') {
	//echo DIR_WS_MODULES . 'UHtmlEmails/'. ULTIMATE_HTML_EMAIL_LAYOUT .'/checkout_process.php';
	$language='espanol';
	$insert_id=$order_id;
	include(DIR_WS_MODULES . 'UHtmlEmails/'. ULTIMATE_HTML_EMAIL_LAYOUT .'/checkout_process.php');
