...
// display number of total products found
	...
	function ajax()
	{
		...
		return '<a rel="nofollow" href="' . tep_href_link( preg_replace( '/^(.+)\//i', '', $_SERVER['SCRIPT_NAME'] ), preg_replace( '/&?page=(\d)+/i', '', $_SERVER['QUERY_STRING'] ) . '&page=' . ($this->current_page_number + 1) ) . '" id="bton-ver-mas">
			<i></i>
			<div>' .  sprintf( PAGINADOR_MOSTRAR, $to_num, $this->number_of_rows) . '</div>
			<span>' . PAGINADOR_MAS . '</span>
		</a>';
	}
  }
?>