...
<table style="width: 100%;" align="center"><tr valign="top"><td>
<?php
...
// end free shipping based on order total
  ...
  //new infoBoxHeading($info_box_contents, false, false);
	echo '<h4>'.CART_SHIPPING_OPTIONS.'</h4>';
  ...
  $ShipTxt.='<table style="width: 100%;" class="estimador">';
  if(sizeof($quotes)) {
    if (tep_session_is_registered('customer_id')) {
      // logged in
  ...
  if (CARTSHIP_SHOWWT == 'true') {
    ...
    $showweight = '';
      ...
      //ishazer remover hard code for version 2.20 : $ShipTxt.='<tr><td class="productListing-data">' . ($total_count == 1 ? ' <strong>Item:</strong></td><td colspan="2" class="productListing-data">' : ' <strong>' . CART_ITEM . '</strong></td><td colspan="2" class="productListing-data">') . $total_count . $showweight . '</td></tr>';
      $ShipTxt.='<tr><td class="productListing-data">' . ($total_count == 1 ? ' <strong>' . CART_ITEM . '</strong></td><td colspan="2" class="productListing-data">' : ' <strong>' . CART_ITEM . '</strong></td><td colspan="2" class="productListing-data">') . $total_count . $showweight . '</td></tr>';
      ...
      if (tep_db_num_rows($addresses_query) > 1){
        ...
        $ShipTxt.='<tr><td colspan="3" class="productListing-data" nowrap class="productListing-data">' .
      ...
      $ShipTxt.='<tr valign="top"><td class="productListing-data"><strong>' . CART_SHIPPING_METHOD_TO .'</strong>&nbsp;</td><td colspan="2" class="productListing-data">'. tep_address_format($order->delivery['format_id'], $order->delivery, 1, ' ', '<br />') . '</td></tr>';
...
// not logged in
      $ShipTxt.=CART_SHIPPING_OPTIONS_LOGIN;
        ...
        if(CARTSHIP_SHOWIC == 'true'){
 //azer for 2.20:      $ShipTxt.='<tr><td class="productListing-data">' . ($total_count == 1 ? ' <strong>Item:</strong></td><td colspan="2" class="productListing-data">' : ' <strong>Items:</strong></td><td colspan="2" class="productListing-data">') . $total_count . $showweight . '</td></tr>';
              $ShipTxt.='<tr><td class="productListing-data" nowrap>' . ($total_count == 1 ? ' <strong>' . CART_ITEM . '</strong></td><td colspan="2" class="productListing-data" nowrap>' : ' <strong>' . CART_ITEM . '</strong></td><td colspan="2" class="productListing-data">') . $total_count . $showweight . '</td></tr>';
      ...
      if($cart->get_content_type() != 'virtual'){
        ...
        $ShipTxt.='<tr><td colspan="3" class="productListing-data" nowrap>' .
                  ENTRY_COUNTRY .'&nbsp;'. tep_get_country_list('country_id', $selected_country,'style="width=200"').'<br />';
        }

//add state zone_id
        $state_array[] = array('id' => '', 'text' => 'Por favor, seleccione...');
...
//        $ShipTxt.='&nbsp;<a href="_" onclick="return shipincart_submit(\'\');">'.CART_SHIPPING_METHOD_RECALCULATE.'</a></td></tr>';
    ...
    if($cart->get_content_type() == 'virtual'){
      ...
      //$ShipTxt.='<tr><td colspan="3" class="productListing-data">'.tep_draw_separator().'</td></tr>';
      $ShipTxt.='<tr><td class="productListing-data" colspan="3">&nbsp;</td></tr><tr><td class="productListing-data" colspan="3"><i>' . CART_SHIPPING_METHOD_FREE_TEXT . ' ' . CART_SHIPPING_METHOD_ALL_DOWNLOADS . '</i></td></tr>';
    }elseif ($free_shipping==1) {
      // order $total is free
      //$ShipTxt.='<tr><td colspan="3" class="productListing-data">'.tep_draw_separator().'</td></tr>';
      $ShipTxt.='<tr><td class="productListing-data" colspan="3">&nbsp;</td></tr><tr><td class="productListing-data" colspan="3"><i>' . sprintf(FREE_SHIPPING_DESCRIPTION, $currencies->format(MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER)) . '</i></td><td>&nbsp;</td></tr>';
    }else{
      // shipping display
	  if ( empty($quotes[0]['error']) || (!empty($quotes[1])&&empty($quotes[1]['error'])) ) {
        $ShipTxt.='<tr><td colspan="3" class="productListing-data">&nbsp;</td></tr><tr><td class="productListing-data"><strong>' . CART_SHIPPING_CARRIER_TEXT . '</strong></td><td class="productListing-data" align="left"><strong>' . CART_SHIPPING_METHOD_TEXT . '</strong></td><td class="productListing-data" align="right"><strong>' . CART_SHIPPING_METHOD_RATES . '</strong></td></tr>';
        $ShipTxt.='<tr><td colspan="3" class="productListing-data">'.tep_draw_separator().'</td></tr>';
      // added to Display Message when No Shipping Options are Available
      $at_least_one_quote_printed = false;
	  } else {
	    $ShipTxt.='<tr><td colspan="3" class="productListing-data">&nbsp;</td></tr>';
	  }
      for ($i=0, $n=sizeof($quotes); $i<$n; $i++) {
        if(sizeof($quotes[$i]['methods'])==1){
          ...
          $ShipTxt.='<td class="productListing-data">'.$quotes[$i]['icon'].'&nbsp;&nbsp;&nbsp;</td>';
          if($quotes[$i]['error']){
            $ShipTxt.='<td colspan="2" class="productListing-data">'.$quotes[$i]['module'].'&nbsp;';
            $ShipTxt.= '('.$quotes[$i]['error'].')</td></tr>';
          }else{
            if($selected_shipping['id'] == $thisquoteid){
             // commented for v2.10 : $ShipTxt.='<td class="productListing-data"><a title="Select this method" href="_"  onclick="return shipincart_submit(\''.$thisquoteid.'\');"><strong>'.$quotes[$i]['module'].'&nbsp;';
$ShipTxt.='<td class="productListing-data"><a title="' . CART_SELECT_THIS_METHOD .'" href="_"  onclick="return shipincart_submit(\''.$thisquoteid.'\');"><strong>'.$quotes[$i]['module'].'&nbsp;';

              $ShipTxt.= '('.$quotes[$i]['methods'][0]['title'].')</strong></a>&nbsp;&nbsp;&nbsp;</td><td align="right" class="productListing-data"><strong>'.$currencies->format(tep_add_tax($quotes[$i]['methods'][0]['cost'], $quotes[$i]['tax'])).'</strong></td></tr>';
            }else{
             // commented for v2.10 : $ShipTxt.='<td class="productListing-data"><a title="Select this method" href="_" onclick="return shipincart_submit(\''.$thisquoteid.'\');">'.$quotes[$i]['module'].'&nbsp;';
 $ShipTxt.='<td class="productListing-data"><a title="' . CART_SELECT_THIS_METHOD .'" href="_" onclick="return shipincart_submit(\''.$thisquoteid.'\');">'.$quotes[$i]['module'].'&nbsp;';

              $ShipTxt.= '('.$quotes[$i]['methods'][0]['title'].')</a>&nbsp;&nbsp;&nbsp;</td><td align="right" class="productListing-data">'.$currencies->format(tep_add_tax($quotes[$i]['methods'][0]['cost'], $quotes[$i]['tax'])).'</td></tr>';
            }
          }
// added to Display Message when No Shipping Options are Available
          ...
          for ($j=0, $n2=sizeof($quotes[$i]['methods']); $j<$n2; $j++) {
            ...
            if($quotes[$i]['error']){
              ...
              if($selected_shipping['id'] == $thisquoteid){
               // commented for v2.10 :  $ShipTxt.='<td class="productListing-data"><a title="Select this method" href="_" onclick="return shipincart_submit(\''.$thisquoteid.'\');"><strong>'.$quotes[$i]['module'].'&nbsp;';
$ShipTxt.='<td class="productListing-data"><a title="' . CART_SELECT_THIS_METHOD .'" href="shopping_cart.php" onclick="return shipincart_submit(\''.$thisquoteid.'\');"><strong>'.$quotes[$i]['module'].'&nbsp;';

                $ShipTxt.= '('.$quotes[$i]['methods'][$j]['title'].')</strong></a>&nbsp;&nbsp;&nbsp;</td><td align="right" class="productListing-data"><strong>'.$currencies->format(tep_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])).'</strong></td><td class="productListing-data">'.tep_image(DIR_WS_ICONS . 'selected.gif', 'Selected').'</td></tr>';
              }else{
              // commented for v2.10 :   $ShipTxt.='<td class="productListing-data"><a title="Select this method" href="_" onclick="return shipincart_submit(\''.$thisquoteid.'\');">'.$quotes[$i]['module'].'&nbsp;';
 $ShipTxt.='<td class="productListing-data"><a title="' . CART_SELECT_THIS_METHOD .'" href="shopping_cart.php" onclick="return shipincart_submit(\''.$thisquoteid.'\');">'.$quotes[$i]['module'].'&nbsp;';

                $ShipTxt.= '('.$quotes[$i]['methods'][$j]['title'].')</a>&nbsp;&nbsp;&nbsp;</td><td align="right" class="productListing-data">'.$currencies->format(tep_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])).'</td><td class="productListing-data">&nbsp;</td></tr>';
          }
            }
          }
          // added to Display Message when No Shipping Options are Available
      ...
      if (!$at_least_one_quote_printed) {
	    $ShipTxt.= '<tr><td colspan="4" class="productListing-data" align="center">'.SHIPPING_ESTIMATOR_NO_OPTIONS_MESSAGE.'</td></tr>';
	  }
    }
  } 
  $ShipTxt.= '</table>
  <table style="width: 100%;"><tr><td align="right"><a href="shopping_cart.php" onclick="return shipincart_submit(\'\');">'. tep_image_button('button_update_cart.gif', IMAGE_BUTTON_UPDATE_CART) . ' </a></td></tr></table>
  </form>';
  ...
  $iva_envios = $selected_shipping['cost']*16/100;
  ...
  if (CARTSHIP_SHOWOT == 'true'){
    ...
    if (!tep_session_is_registered('customer_id')){
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        $order->info['tax'] += $tax_val + $iva_envios;
 ...
 // $info_box_contents[] = array('text' => '<strong>' . CART_OT . '</strong>'); //azer version 2.20
    ...
    new infoBoxHeading($info_box_contents, false, false);
    $otTxt='<table align="right" class="order_total">';
