...
<br />For example:<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>Product Cost:</strong>&nbsp; ' .  $currencies->format(100) . '<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>Value of Points Earned:</strong>&nbsp; ' .  $currencies->format(tep_calc_shopping_pvalue(100 * POINTS_PER_AMOUNT_PURCHASE)) . '<br /><br />
...
if (POINTS_LIMIT_VALUE  > 0)  {
	define('TEXT_FAQ_5', 'Currently, a minimum balance of <strong>' . number_format(POINTS_LIMIT_VALUE) . '</strong> points <strong>(' . $currencies->format(tep_calc_shopping_pvalue(POINTS_LIMIT_VALUE)) . ')' . '</strong> is required before you can redeem them.
...
if(tep_not_null(POINTS_MIN_AMOUNT))  {
	define('TEXT_FAQ_6', 'Currently, a minimum of <strong>' . $currencies->format(POINTS_MIN_AMOUNT) . '</strong> in total (per purchase) is required before any Points Redemptions can take place.
...
// FAQ7
define('TEXT_FAQ_7', 'A maximum of <strong>' . number_format(POINTS_MAX_VALUE) . '</strong> points <strong>(' . $currencies->format(tep_calc_shopping_pvalue(POINTS_MAX_VALUE)) . ')' . '</strong> is allowed to redeem per order.
<br /><br />We strongly advise you to check this page often as we may make changes to this policy.
...
if (tep_not_null(USE_REFERRAL_SYSTEM)){
	...
	<br />When we receive your referred friends completed and approved order, we will reward your Points account with <strong>' .  USE_REFERRAL_SYSTEM . '</strong> points .
...
if (tep_not_null(USE_POINTS_FOR_REVIEWS)){
	...
	<br />We would like to thank you for helpful review that you gave us, therefore for every quality review, we will reward your Points account with <strong>' .  $currencies->format(tep_calc_shopping_pvalue(USE_POINTS_FOR_REVIEWS)) . '</strong> worth of points .
...
if (tep_not_null(RESTRICTION_MODEL)) {
	define('TEXT_FAQ_14', 'Currently, only items which have the model <strong>[' . RESTRICTION_MODEL . ']</strong> can be purchased using your points balance.
	<br /><br />We strongly advise you to check this page often as we may make changes to this policy.
...
define('TEXT_INFORMATION', '<a name="Top"></a><span class="pointWarning"><strong>Please choose from one of the topics below:</strong></span>
<ol>
